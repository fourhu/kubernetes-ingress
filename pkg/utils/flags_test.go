...
func TestOSArgs_ValidateNamespaceSelectorReadyTimeout(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{name: "zero allowed", args: OSArgs{NamespaceLabelSelector: "watch=true"}},
		{
			name: "positive allowed",
			args: OSArgs{NamespaceLabelSelector: "watch=true", NamespaceSelectorReadyTimeout: time.Minute},
		},
		{
			name:    "negative rejected",
			args:    OSArgs{NamespaceLabelSelector: "watch=true", NamespaceSelectorReadyTimeout: -time.Second},
			wantErr: true,
		},
		{
			name: "negative ignored without selector",
			args: OSArgs{NamespaceSelectorReadyTimeout: -time.Second},
		},
		{
			name: "negative ignored when whitelist set",
			args: OSArgs{
				NamespaceLabelSelector:        "watch=true",
				NamespaceWhitelist:            []string{"app"},
				NamespaceSelectorReadyTimeout: -time.Second,
			},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := tt.args.ValidateNamespaceLabelSelector()
			if (err != nil) != tt.wantErr {
				t.Fatalf("ValidateNamespaceLabelSelector() error = %v, wantErr %v", err, tt.wantErr)
			}
			if err != nil && !strings.Contains(err.Error(), "--namespace-selector-ready-timeout") {
				t.Errorf("error %q should name --namespace-selector-ready-timeout", err)
			}
		})
	}
}
...
func TestOSArgs_NamespaceLabelSelectorCanonicalAndIgnored(t *testing.T) {
	...
	if ignored.NamespaceLabelSelectorCanonical() != "" {
		t.Error("Canonical() with whitelist must be empty")
	}
