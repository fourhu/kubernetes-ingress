...
type OSArgs struct {
	...
	NamespaceLabelSelector            string         `long:"namespace-label-selector" description:"label selector to dynamically watch namespaces. Matching namespaces get sticky per-namespace watches; an unlabeled namespace keeps its watch but is dropped from the config until it matches again. The --configmap namespace is always watched. Ignored if --namespace-whitelist or --namespace-blacklist is set"`
	NamespaceSelectorReadyTimeout     time.Duration  `long:"namespace-selector-ready-timeout" default:"30s" description:"how long bootstrap waits for selected namespaces to become ready before the first config sync. 0 waits indefinitely. Negative values are rejected. Only used with --namespace-label-selector"`
...
// the active filter. Whitespace-only or invalid selectors fail startup, as does
// a negative --namespace-selector-ready-timeout (which would otherwise wait
// forever because every timeout check is gated on timeout > 0).
func (a OSArgs) ValidateNamespaceLabelSelector() error {
	if a.NamespaceLabelSelectorActive() && a.NamespaceSelectorReadyTimeout < 0 {
		return fmt.Errorf("invalid --namespace-selector-ready-timeout %s: must not be negative", a.NamespaceSelectorReadyTimeout)
	}
