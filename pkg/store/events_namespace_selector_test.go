...
func TestSelectorLateEventsAfterUnmatchDoNotStick(t *testing.T) {
	...
	for _, status := range []Status{ADDED, MODIFIED} {
		if k.EventGateway(detached, &Gateway{Namespace: "app", Name: "gateway", Status: status}) {
			t.Fatalf("late Gateway %s on detached ns must be dropped", status)
		}
		if k.EventTCPRoute(detached, &TCPRoute{Namespace: "app", Name: "route", Status: status}) {
			t.Fatalf("late TCPRoute %s on detached ns must be dropped", status)
		}
		if k.EventReferenceGrant(detached, &ReferenceGrant{Namespace: "app", Name: "grant", Status: status}) {
			t.Fatalf("late ReferenceGrant %s on detached ns must be dropped", status)
		}
	}
	if len(detached.Gateways) != 0 || len(detached.TCPRoutes) != 0 || len(detached.ReferenceGrants) != 0 {
		t.Fatal("late Gateway API events must leave detached namespace maps untouched")
	}
	if _, ok := k.Namespaces["app"]; ok {
		t.Fatal("late Gateway API events must not resurrect namespace")
	}
}

func TestSelectorGatewayEventsPreserveDeletesForPersistedNamespace(t *testing.T) {
	k := selectorStore()
	ns := addSelectedNamespace(&k, "app")
	k.EventGateway(ns, &Gateway{Namespace: "app", Name: "gateway", Status: ADDED})
	k.EventTCPRoute(ns, &TCPRoute{Namespace: "app", Name: "route", Status: ADDED})
	k.EventReferenceGrant(ns, &ReferenceGrant{Namespace: "app", Name: "grant", Status: ADDED})

	if !k.EventGateway(ns, &Gateway{Namespace: "app", Name: "gateway", Status: DELETED}) {
		t.Fatal("Gateway DELETE in a persisted namespace must be retained")
	}
	if !k.EventTCPRoute(ns, &TCPRoute{Namespace: "app", Name: "route", Status: DELETED}) {
		t.Fatal("TCPRoute DELETE in a persisted namespace must be retained")
	}
	if !k.EventReferenceGrant(ns, &ReferenceGrant{Namespace: "app", Name: "grant", Status: DELETED}) {
		t.Fatal("ReferenceGrant DELETE in a persisted namespace must be retained")
	}
	if ns.Gateways["gateway"].Status != DELETED || ns.TCPRoutes["route"].Status != DELETED {
		t.Fatal("Gateway and TCPRoute deletes must remain available for reconciliation")
	}
	if _, ok := ns.ReferenceGrants["grant"]; ok {
		t.Fatal("ReferenceGrant DELETE must remove the stored grant")
	}
}
...
func TestSelectorUnmatchClearsProcessedResourceVersion(t *testing.T) {
	k := selectorStore()
	ns := addSelectedNamespace(&k, "app")
	ing := selectorIngress("app", "web", "app", "svc")
	ing.UID = types.UID("uid-unmatch-clears")
	if !k.EventIngress(ns, ing, ing.UID, "1") {
		t.Fatal("ingress ADD should report a change")
	}
	processed := &meta.GetMetaStore().ProcessedResourceVersion
	if !processed.IsProcessed(ing, ing.UID, "1") {
		t.Fatal("ingress should be marked processed after ADD")
	}

	k.EventNamespace(ns, &Namespace{Name: "app", Status: DELETED})

	if processed.IsProcessed(ing, ing.UID, "1") {
		t.Fatal("teardown must clear ProcessedResourceVersion via the ingress UID")
	}
	if _, ok := k.Namespaces["app"]; ok {
		t.Fatal("unmatched namespace must be removed from the store")
	}
}

func TestEventTCPCRNilIsNoop(t *testing.T) {
	t.Parallel()
	// A typed-nil *TCPs in the event interface passes the producer's
	// `job.Data != nil` check; the store must not dereference it.
	for _, k := range []K8s{selectorStore(), NewK8sStore(utils.OSArgs{})} {
		if k.EventTCPCR("app", "tcp", nil) {
			t.Fatal("EventTCPCR(nil) must not report a change")
		}
	}
}

func TestConfigMapNamespaceAlwaysSelected(t *testing.T) {
	t.Parallel()
	k := NewK8sStore(utils.OSArgs{
		NamespaceLabelSelector: "watch=true",
		ConfigMap:              utils.NamespaceValue{Namespace: "base", Name: "cm"},
	})
	if !k.NamespaceAlwaysSelected("base") {
		t.Fatal("configmap namespace must be always selected")
	}
	if _, ok := k.NamespacesAccess.Selected["base"]; !ok {
		t.Fatal("configmap namespace must be in the selected set")
	}
	// It is watched but, without matching labels, stays non-Relevant like the
	// automatic configmap namespace of whitelist mode.
	k.EventNamespace(nil, &Namespace{Name: "base", Status: ADDED, Labels: map[string]string{}})
	if k.MarkNamespaceReady("base") {
		t.Fatal("always-selected namespace must not become Relevant without matching labels")
	}
	if k.Namespaces["base"].Relevant {
		t.Fatal("always-selected namespace must stay non-Relevant")
	}
	// Its Services remain readable for the built-in local default backend.
	k.EventService(k.Namespaces["base"], &Service{Namespace: "base", Name: "svc", Status: ADDED})
	if _, err := k.GetService("base", "svc"); err != nil {
		t.Fatalf("always-selected namespace services must be readable: %v", err)
	}
}
