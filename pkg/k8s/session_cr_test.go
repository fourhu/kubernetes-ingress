...
func sessionCRClient(t *testing.T, lists chan<- string) *k8s {
	...
	k := &k8s{
		builtInClient: kubernetes.NewForConfigOrDie(cfg),
		crClientV1:    crclientv1.NewForConfigOrDie(cfg),
		crClientV3:    crclientv3.NewForConfigOrDie(cfg),
...
func (cr *countingSessionCR) GetInformerV3(
	ch chan k8ssync.SyncDataEvent,
	factory crinformersv3.SharedInformerFactory,
	args utils.OSArgs,
) (cache.SharedIndexInformer, cache.ResourceEventHandlerRegistration) {
...
func TestLateCRSyncDoesNotBlockNamespacesOrFutureCRDs(t *testing.T) {
	lists := make(chan string, 100)
	k := sessionCRClient(t, lists)
	// BARRIER has no namespace; process-wide epochs identify its session of origin.
	...
	k.sessions.mu.RLock()
	badEpoch := k.sessions.sessions["bad"].epoch
	healthyEpoch := k.sessions.sessions["healthy"].epoch
	k.sessions.mu.RUnlock()
	stop := make(chan struct{})
	defer close(stop)
	for _, kind := range []string{"TCP", "Backend"} {
		...
		// BARRIER could hide a missing or prematurely emitted TCP drain.
		...
		for len(drained) < wantDrains {
			...
			case ev := <-k.sessions.eventChan:
				...
				case k8ssync.BARRIER:
					ns := "bad"
					if ev.NamespaceEpoch == healthyEpoch {
						ns = "healthy"
					} else if ev.NamespaceEpoch != badEpoch {
