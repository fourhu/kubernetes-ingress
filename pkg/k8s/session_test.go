...
func TestSessionManagerMarkReadyAndClose(t *testing.T) {
	...
	if !m.WaitAllReady(make(chan struct{}), 0) {
...
func TestDrainSessionEventsWaitsForPriorProxyEvents(t *testing.T) {
	...
	if second.SyncType != k8ssync.BARRIER {
		t.Fatalf("second = %s, want BARRIER", second.SyncType)
	}
	if second.EventProcessed == nil {
		t.Fatal("drain BARRIER must carry EventProcessed")
	}
	close(second.EventProcessed)
	select {
...
func TestDrainSessionEventsReturnsOnSessionStop(t *testing.T) {
	...
	select {
	case <-process:
		t.Fatal("stopped session must not send a drain BARRIER")
	default:
	}
}
...
func TestWaitAllReadyReturnsWhenSessionReady(t *testing.T) {
	t.Parallel()
	ch := make(chan k8ssync.SyncDataEvent, 8)
	m := newSessionManager(ch, fakeStarter())
	if err := m.Start("app"); err != nil {
	...
	go func() {
		done <- m.WaitAllReady(nil, 0)
	}()
	select {
	case <-done:
		t.Fatal("WaitAllReady returned while the session was still starting")
	case <-time.After(100 * time.Millisecond):
	}

	// Production flips the phase under the manager lock after SyncData
	// consumed the READY event.
	m.mu.Lock()
	...
	case ready := <-done:
		if !ready {
			t.Fatal("WaitAllReady must be true once no session is starting")
		}
	case <-time.After(2 * time.Second):
		t.Fatal("WaitAllReady did not return after the session became ready")
	}
...
func TestWaitAllReadyTimesOutWhileStarting(t *testing.T) {
	origLog := waitAllReadyStuckLogInterval
	waitAllReadyStuckLogInterval = time.Hour
	t.Cleanup(func() {
		waitAllReadyStuckLogInterval = origLog
	})

	ch := make(chan k8ssync.SyncDataEvent, 8)
	m := newSessionManager(ch, fakeStarter())
	if err := m.Start("stuck"); err != nil {
		t.Fatal(err)
	}
	done := make(chan bool, 1)
	go func() {
		done <- m.WaitAllReady(nil, 50*time.Millisecond)
	}()
	select {
	case ready := <-done:
		if !ready {
			t.Fatal("timeout must continue bootstrap")
		}
	case <-time.After(2 * time.Second):
		t.Fatal("WaitAllReady did not return after timeout")
	}
	select {
	case ev := <-ch:
		if ev.EventProcessed != nil {
...
func TestDrainConcurrentCloseCallsShutdownOnce(t *testing.T) {
	t.Parallel()
	events := make(chan k8ssync.SyncDataEvent, 4)
	shutdownEntered := make(chan struct{}, 2)
	releaseShutdown := make(chan struct{})
	shutdownDone := make(chan struct{})
	m := newSessionManager(events, func(namespace string, epoch uint64, stopCh chan struct{}) (*nsSession, error) {
		return &nsSession{
			namespace: namespace,
			epoch:     epoch,
			stopCh:    stopCh,
	...
	ready := <-events
	if ready.SyncType != k8ssync.NAMESPACE_SESSION_READY {
		t.Fatalf("event = %s, want READY", ready.SyncType)
	}
	close(ready.EventProcessed)

	if !m.Drain("app") {
		t.Fatal("existing session must start draining")
	}
	select {
	case <-shutdownEntered:
	case <-time.After(2 * time.Second):
		t.Fatal("drain did not enter shutdown")
	}
	m.Close()
	select {
	case <-shutdownEntered:
		close(releaseShutdown)
		t.Fatal("Drain and Close invoked shutdown concurrently")
	case <-time.After(100 * time.Millisecond):
	}
	close(releaseShutdown)
	select {
	...
	retired := <-events
	if retired.SyncType != k8ssync.NAMESPACE_WATCH_RETIRED {
		t.Fatalf("event = %s, want RETIRED", retired.SyncType)
	}
	close(retired.EventProcessed)
}
