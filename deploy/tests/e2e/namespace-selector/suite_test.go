...
type NamespaceSelectorSuite struct {
	...
	foreignNS        string
	originalArgs     []string
	originalStrategy json.RawMessage
}
...
func (suite *NamespaceSelectorSuite) SetupSuite() {
	...
	suite.originalArgs, suite.originalStrategy, err = controllerConfig()
	suite.Require().NoError(err)
	suite.T().Cleanup(func() {
		if err := restoreControllerArgs(suite.originalArgs); err != nil {
			suite.T().Errorf("failed to restore controller args: %v", err)
		}
		if err := setControllerStrategy(suite.originalStrategy); err != nil {
			suite.T().Errorf("failed to restore controller strategy: %v", err)
		}
	})
	suite.Require().NoError(setControllerStrategy(json.RawMessage(`{"type":"Recreate"}`)))

	// Default backend and ConfigMap live in the controller namespace, which
	// selector mode always watches (mirroring --namespace-whitelist). No label
	// is needed there, so this deliberately leaves it unlabeled to exercise the
	// automatic base namespace.
	...
	out, err := kubectl("delete", "ns", suite.foreignNS, "--ignore-not-found=true", "--wait=true", "--timeout=120s")
...
func controllerConfig() ([]string, json.RawMessage, error) {
	...
	if len(deploy.Spec.Template.Spec.Containers) == 0 {
		return nil, nil, errors.New("controller deployment has no containers")
	}
	return deploy.Spec.Template.Spec.Containers[0].Args, deploy.Spec.Strategy, nil
}

func setControllerStrategy(strategy json.RawMessage) error {
	patch := `[{"op":"replace","path":"/spec/strategy","value":` + string(strategy) + "}]"
	_, err := kubectl("-n", controllerNS, "patch", "deploy", controllerName, "--type=json", "-p", patch)
	return err
}
